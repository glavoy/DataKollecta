...
Future<void> _build(List<String> args) async {
  ...
  try {
    ...
    switch (target) {
      ...
      case 'windows':
        _writeInstallerVersion(version);
...
/// Writes the version into an Inno Setup include, so the Windows installer
/// cannot drift from pubspec.yaml.
///
/// installer.iss used to carry its own hard-coded `MyAppVersion`, which sat at
/// 1.0.3+2 while the app shipped 1.2.0+8 -- every installer built in between
/// was mislabelled in Add/Remove Programs and in its own filename. A second
/// hand-synced copy of a version number will always drift, so the build writes
/// it instead.
void _writeInstallerVersion(String version) {
  final file = File('windows/installer_version.iss');
