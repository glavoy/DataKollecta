...
- **System variables no longer have to be declared.** `starttime`, `startdate`, `uniqueid`, `swver`, `survey_id`, `lastmod` and `stoptime` are added to any questionnaire that does not list them, in the positions that make them correct: an automatic question records its value when navigation reaches it, so `starttime` goes before the first question and `stoptime` after the last. Questionnaires that already declare them are untouched, so every existing survey package behaves exactly as before. Data dictionaries may keep declaring them — useful for documenting the variables to analysts — and the generator now says so rather than failing.
