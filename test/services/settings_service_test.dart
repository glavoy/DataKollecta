...
void main() {
  ...
  group('sync batch size', () {
    test('defaults to 25 when nothing has been stored', () async {
      expect(await SettingsService().syncBatchSize,
          SettingsService.defaultSyncBatchSize);
      expect(SettingsService.defaultSyncBatchSize, 25);
    });

    test('round-trips a value inside the range', () async {
      final settings = SettingsService();
      await settings.setSyncBatchSize(50);

      expect(await settings.syncBatchSize, 50);
    });

    test('clamps a stored value that is out of range on the way back out',
        () async {
      // The read clamps as well as the write, so a value that reached the
      // store some other way -- an older build, a hand-edited prefs file --
      // still cannot post an unbounded batch. app-sync refuses more than 500
      // rows, and a 413 stops a sync run after three consecutive failures
      // with a message no interviewer can act on.
      SharedPreferences.setMockInitialValues({'sync_batch_size': '100000'});
      expect(await SettingsService().syncBatchSize,
          SettingsService.maxSyncBatchSize);

      SharedPreferences.setMockInitialValues({'sync_batch_size': '0'});
      expect(await SettingsService().syncBatchSize,
          SettingsService.minSyncBatchSize);
    });

    test('an unparseable stored value falls back to the default', () async {
      SharedPreferences.setMockInitialValues({'sync_batch_size': 'lots'});

      expect(await SettingsService().syncBatchSize,
          SettingsService.defaultSyncBatchSize);
    });

    test('the clamp never lets a value through that app-sync would refuse',
        () {
      // 500 is app-sync's MAX_BATCH_ROWS. The client ceiling is deliberately
      // well under it, so a 413 can only mean a caller that is not this app.
      expect(SettingsService.maxSyncBatchSize, lessThan(500));
      for (final value in [-5, 0, 1, 25, 200, 201, 999999]) {
        final clamped = SettingsService.clampSyncBatchSize(value);
        expect(clamped, greaterThanOrEqualTo(SettingsService.minSyncBatchSize));
        expect(clamped, lessThanOrEqualTo(SettingsService.maxSyncBatchSize));
      }
    });
  });
